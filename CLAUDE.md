...
- **Build truth (from `Pomo TAP.xcodeproj/project.pbxproj`):** `WATCHOS_DEPLOYMENT_TARGET = 26.0`, `SWIFT_VERSION = 5.0`. The target was raised from 11.6 to 26.0 to adopt the watchOS-26 `ControlWidget` and Smart-Stack `RelevanceConfiguration` APIs (both are `@available(watchOS 26.0)` and would not compile below it). Use these as ground truth. The codebase follows current watchOS HIG and prefers modern APIs (WidgetKit over ClockKit, `DispatchSourceTimer`, system `Gauge`/`ProgressView`). When using an API newer than training, verify signatures against the installed SDK's `.swiftinterface` — don't write WWDC framework API from memory.
...
When changing transition behavior, modify `prepareNextPhase` / `shouldClearNotifications`, not the call sites. The legacy `moveToNextPhase(autoStart:skip:)` is a thin wrapper kept for compatibility — prefer `prepareNextPhase`.
...
- Action `START_NEXT_PHASE` (category `PHASE_COMPLETED`) → `TimerModel.handleNotificationResponse()`, which is idempotent: if the phase already auto-advanced it just starts the timer rather than advancing again.
